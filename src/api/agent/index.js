...
module.exports = (db) => {
  ...
  //GET /agent/travelGroup/:name
  router.get('/travelGroup/:name', async(req, res) => {
    try {
      const result = await Agent.getByGroup(req.params.name.toLowerCase()).toArray()
      if(result.length > 0){
        let finalResult = result.map((el) => {
          delete el.password
          delete el.purchaseDivisionHead
          delete el.accountDivisionHead
          return el
        })
